...
test('hook timing is measured against the issued cast', () => {
  ...
  assert.throws(() => hookCast(cast, cast.biteAt - 1, 42)); assert.throws(() => hookCast(cast, cast.expiresAt + 1, 42));
  const hooked = hookCast(cast, cast.biteAt + 100, 42);
  assert.equal(hooked.hookedAt, cast.biteAt + 100); assert.throws(() => hookCast(hooked, cast.biteAt + 200, 42));
});
test('server awards a completed transcript once and rejects fast-forwarded play', () => {
  ...
  while (s.status === 'playing') {
    const next: boolean = s.tension > .7 ? false : s.tension < .35 ? true : hold;
    if (hold !== next) { hold = next; events.push({ at: s.elapsed, hold }); }
    s = stepReel(s, hold, 42);
  }
  const p = newProfile(0), { cast } = createCast(p, 0, [0.1, 0.5, 0.5], 'cast-3');
...
test('failed reels and expired hooked casts never create rewards', () => {
  const p = newProfile(0), { cast } = createCast(p, 0, [0.1, 0.5, 0.5], 'cast-4');
