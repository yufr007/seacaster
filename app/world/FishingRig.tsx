...
export function FishingRig({ phase, motion, reduced, home, platform, species }: Pick<WorldProps, 'phase' | 'motion' | 'reduced' | 'home' | 'platform' | 'species'>) {
  ...
  useFrame(({ clock }, dt) => {
    if (!rod.current || !float.current) return;
    ...
    bendRef.current = bend; holdingRef.current = m.holding;
    ...
    if (flying) {
      ...
      vectors.bob.y = waveHeight(vectors.bob.x, vectors.bob.z, time) + .06;
  ...
  return <>
    <group ref={rod}><ArtRod bend={bendRef} holding={holdingRef} reduced={reduced} golden={gold || rodId === 'gold'} /></group>
