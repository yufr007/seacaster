...
test('tackle uses catalogue prices and does not mutate the input', () => {
  const p = newProfile(0);
  const q = buyTackle(p, 'shrimp');
  assert.equal(q.coins, 60);
  assert.equal(q.baits.shrimp, 5);
  assert.equal(p.coins, 100);
  assert.throws(() => buyTackle(p, 'invented'));
  assert.throws(() => buyTackle({ ...p, coins: 0 }, 'shrimp'));
});
test('catch rarity and weight are selected by rules, not caller-provided attributes', () => {
  assert.equal(chooseCatch(0, 0).speciesId, 'f1');
  assert.equal(chooseCatch(0.9999, 0.99).speciesId, 'f15');
...
test('a catch updates collection once and advances daily progress', () => {
  const p = newProfile(0);
  const fish = chooseCatch(0, 0.5);
  const q = awardCatch(p, fish, 0);
  assert.equal(q.catches.f1.count, 1);
  assert.equal(q.totalCatches, 1);
  assert.equal(q.daily.catches, 1);
  assert.equal(q.xp, FISH[0].xp);
  assert.equal(p.totalCatches, 0);
});
test('daily reward cannot be claimed twice or before completion', () => {
  let p = newProfile(0);
  assert.throws(() => claimDaily(p, 0));
  for (let i = 0; i < 5; i++) p = awardCatch(p, chooseCatch(0, 0.5), 0);
  const q = claimDaily(p, 0);
  assert.equal(q.coins, p.coins + 100);
  assert.throws(() => claimDaily(q, 0));
  assert.throws(() => claimDaily(q, 86400000));
});
test('tampered and malformed local saves reset without granting entitlements', () => {
  const p = newProfile(0);
  assert.deepEqual(validateProfile(p, 0), p);
  ...
  assert.equal('premium' in clean, false);
  assert.equal('arbitrary' in clean, false);
});
...
test('input replay matches the actual deterministic reel simulation', () => {
  ...
  while (s.status === 'playing' && s.elapsed < 30000) {
    const next = s.tension > 0.7 ? false : s.tension < 0.35 ? true : hold;
    if (next !== hold) { hold = next; events.push({ at: s.elapsed, hold }); }
    s = stepReel(s, hold, 42);
  }
  assert.equal(s.status, 'won');
  assert.deepEqual(replayReel(42, events, s.elapsed), s);
});
