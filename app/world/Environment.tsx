...
function Sky({ sky, reduced }: Pick<WorldProps, 'sky' | 'reduced'>) {
  ...
  return <>
    ...
    <group position={[sky.sun ? -7 : 7, 10 + day * 2, -34]}>
    ...
    <group ref={clouds}>
      {[[-13, 8, -28, 1.1], [7, 10, -32, 1.2], [16, 7, -26, .9], [-2, 11, -43, 1]].map(([x, y, z, s], i) => <group key={i} position={[x, y, z]} scale={s}>
...
function SwimmingSchool({ reduced }: { reduced: boolean }) {
  const school = useRef<Group>(null);
  useFrame(({ clock }) => {
    if (!school.current) return;
    const t = reduced ? 2 : clock.elapsedTime;
    school.current.children.forEach((child, i) => { child.position.set(Math.sin(t * .2 + i * 1.6) * 3.6, -.45, -3 - i * 1.5 + Math.cos(t * .23 + i) * .8); child.rotation.y = Math.cos(t * .2 + i * 1.6) > 0 ? 0 : Math.PI; });
  });
...
export function Environment({ sky, reduced, platform }: Pick<WorldProps, 'sky' | 'reduced' | 'platform'>) {
  return <><Sky sky={sky} reduced={reduced} /><Island /><Island right /><PassingBoat reduced={reduced} />{[0, 1, 2].map(i => <Gull key={i} index={i} reduced={reduced} />)}<SwimmingSchool reduced={reduced} />{platform === 'river' && <group>{[-1, 1].map(side => <group key={side} position={[side * 6, -.12, -6]}><Pebble at={[0, 0, 0]} size={[3.7, .45, 11]} color="#7cc48a" /><Pebble at={[-side * 2.2, .1, 4]} size={[1.2, .5, 1.8]} color="#b3c5a6" /><Palm at={[0, .3, -1]} scale={1.35} />{Array.from({ length: 7 }, (_, i) => <mesh key={i} position={[-side * 2.7, .45, i * 1.4 - 3]} rotation={[0, i, side * .13]}><coneGeometry args={[.13, 1.25, 5]} /><meshStandardMaterial color={i % 2 ? '#429d70' : '#a9d676'} /></mesh>)}</group>)}{[[-2, .03, -3], [2.5, .03, -5], [-3, .03, -8]].map((p, i) => <mesh key={i} position={p as [number, number, number]} rotation={[-Math.PI / 2, 0, i]}><circleGeometry args={[.45, 18, .2, Math.PI * 1.8]} /><meshStandardMaterial color="#a4d974" side={2} /></mesh>)}</group>}</>;
}
